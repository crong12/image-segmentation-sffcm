...
% Output:
% fs is the result of segmentation
% center_p is the center pixel of each labelled region
f=im2double(f);
...
if size(f,3)<2
    ...
    for i=1:num_area
        f2=f(s3==i);f_med=median(f2);fx=im2double((s3==i))*im2double(f_med);
        fs=fs+fx;
        center_p(i,:)=f_med;
        Num_p=zeros(num_area,1);
    end
...
TT2=fcm_algo.colorspace('Lab<-RGB',TT);
